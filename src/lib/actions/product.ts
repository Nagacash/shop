...
export async function getAllProducts(filters: NormalizedProductFilters): Promise<GetAllProductsResult> {
  ...
  const priceAgg = {
    ...
    maxInStock: sql<number | null>`max(${variantJoin.inStock})`,
  };
  ...
  const productsOut: ProductListItem[] = rows.map((r) => ({
    ...
    subtitle: [r.categoryLabel, r.subtitle].filter(Boolean).join(" · ") || null,
    soldOut: r.maxInStock === null ? false : Number(r.maxInStock) <= 0,
  }));
...
export async function getFeaturedProduct(name: string): Promise<FeaturedProduct | null> {
  const [product] = await db
    ...
    .from(products)
    .where(and(eq(products.isPublished, true), eq(products.name, name)))
    .limit(1);
  ...
  const soldOut =
    data.variants.length > 0 && data.variants.every((v) => (v.inStock ?? 0) <= 0);

  return {
    id: data.product.id,
    name: data.product.name,
    description: data.product.description,
    imageUrl: normalizeImageUrl(primaryImage?.url) ?? FALLBACK_PRODUCT_IMAGE,
    minPrice: prices.length ? Math.min(...prices) : null,
    subtitle:
      [data.product.category?.name, data.product.gender?.label].filter(Boolean).join(" · ") ||
      null,
    soldOut,
  };
}
